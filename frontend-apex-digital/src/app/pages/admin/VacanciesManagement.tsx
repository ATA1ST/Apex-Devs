...
type VacancyStatus = 'active' | 'closed';
...
function mapVacancyToForm(vacancy: Vacancy): VacancyFormState {
  return {
    ...
    status: (vacancy.status as VacancyStatus) || 'active',
...
function normalizeApplicationsFromApi(raw: any[]): JobApplication[] {
  return (raw || []).map((item) => ({
    ...
    resumeFileName:
      item.resumeFileName ?? item.resume?.fileName ?? item.resume?.name,
...
export function VacanciesManagement() {
  ...
  const adminJobsBaseUrl = useMemo(() => `${API_BASE_URL}/api/admin/jobs`, []);
  const adminApplicationsBaseUrl = useMemo(
  ...
  const loadVacancies = async () => {
    try {
      ...
      const response = await fetch(adminJobsBaseUrl, {
        method: 'GET',
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        editingVacancyId ? `${adminJobsBaseUrl}/${editingVacancyId}` : adminJobsBaseUrl,
        {
      ...
      if (!response.ok) {
        let errorMessage = editingVacancyId
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`${adminJobsBaseUrl}/${id}`, {
        method: 'DELETE',
      });
  ...
  const toggleVisibility = async (vacancy: Vacancy) => {
    ...
    try {
      const response = await fetch(`${adminJobsBaseUrl}/${vacancy.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          requirements: payload.description.ru.requirements.map((req, index) => ({
            ru: req,
            kz: payload.description.kz.requirements[index] || '',
            en: payload.description.en.requirements[index] || '',
          })),
      ...
      if (!response.ok) {
        throw new Error('Не удалось изменить видимость вакансии');
      }

      setVacancies((prev) =>
        prev.map((v) => (v.id === vacancy.id ? { ...v, isVisible: !v.isVisible } : v))
      );
  ...
  const showApplications = async (vacancy: Vacancy) => {
    ...
    try {
      ...
      const response = await fetch(`${adminJobsBaseUrl}/${vacancy.id}/applications`, {
        method: 'GET',
      });
  ...
  const downloadResume = async (app: JobApplication) => {
    try {
      ...
      const response = await fetch(`${adminApplicationsBaseUrl}/${app.id}/resume`, {
        method: 'GET',
      });

      if (!response.ok) {
        throw new Error('Не удалось получить ссылку на резюме');
      }
              ...
              <div key={vacancy.id} className="bg-white rounded-lg border p-6 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <Badge variant={vacancy.isVisible ? 'default' : 'secondary'}>
                        {vacancy.isVisible ? 'Опубликовано' : 'Скрыто'}
                      </Badge>
                      {(vacancy.applicationsCount ?? 0) > 0 && (
                        <Badge
                          variant="destructive"
                          className="cursor-pointer"
                          onClick={() => showApplications(vacancy)}
                        >
                  ...
                  <div className="flex items-center space-x-2 ml-4">
                    ...
                    <Button size="sm" variant="outline" onClick={() => toggleVisibility(vacancy)}>
                    ...
                    <Button size="sm" variant="destructive" onClick={() => handleDelete(vacancy.id)}>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="visible">Опубликовано</SelectItem>
